...
class JobMatchingEngine:
    ...
    def __init__(self):
        ...
        self.weights = {
            ...
            'experience': 0.25,   # Experience level and years
            'location': 0.15,     # Location preferences
            'salary': 0.10,      # Salary expectations
            'company_fit': 0.10, # Company culture and size
            'work_type': 0.05,   # Remote, full-time, etc.
        }
    ...
    async def find_matches(
        ...
        try:
            ...
            for job in job_postings:
                match_analysis = await self._analyze_match(user_profile, job)

                if match_analysis['overall_score'] >= min_score:
                    recommendation = JobRecommendation(
                        job_posting=job,
                        match_score=match_analysis['overall_score'] * 100,
                        match_reasons=match_analysis['reasons'],
                        skill_matches=match_analysis['skill_matches'],
                        skill_gaps=match_analysis['skill_gaps'],
                        salary_fit=match_analysis['salary_fit'],
                        location_fit=match_analysis['location_fit'],
                        experience_fit=match_analysis['experience_fit'],
                    )
    ...
    async def analyze_compatibility(
        self, user_id: str, job_id: str, job_posting: JobPosting
    ) -> Dict[str, Any]:
        """
        Analyze detailed compatibility between a user and specific job.
        ...
        try:
            user_profile = await self._get_user_profile(user_id)
            match_analysis = await self._analyze_match(user_profile, job_posting)

            return {
                'user_id': user_id,
                'job_id': job_id,
                'overall_score': match_analysis['overall_score'],
                'detailed_scores': match_analysis['detailed_scores'],
            ...
            logger.error(f"Failed to analyze compatibility: {str(e)}")
            return {}
    ...
    async def analyze_skill_gap(
        ...
        try:
            ...
            if target_role:
                ...
                required_skills = await self._get_career_skills(user_profile.preferences)
    ...
    async def generate_career_roadmap(
        ...
        try:
            user_profile = await self._get_user_profile(user_id)
            skill_gap = await self.analyze_skill_gap(user_id, target_role)
    ...
    async def _analyze_match(self, user_profile: UserProfile, job: JobPosting) -> Dict[str, Any]:
        """Analyze detailed match between user and job."""

        # Calculate individual scores
        skill_score = self._calculate_skill_score(user_profile, job)
        experience_score = self._calculate_experience_score(user_profile, job)
        location_score = self._calculate_location_score(user_profile, job)
        salary_score = self._calculate_salary_score(user_profile, job)
        company_score = self._calculate_company_score(user_profile, job)
        work_type_score = self._calculate_work_type_score(user_profile, job)
        ...
        return {
            ...
            'reasons': reasons,
            'salary_fit': salary_score > 0.5,
            'location_fit': location_score > 0.5,
            'experience_fit': experience_score > 0.5,
            'strengths': self._identify_strengths(user_profile, job),
            'weaknesses': self._identify_weaknesses(user_profile, job),
            'recommendations': self._generate_recommendations(user_profile, job),
        }
    ...
    def _generate_match_reasons(
        self, user_profile: UserProfile, job: JobPosting, scores: Dict[str, float]
    ) -> List[str]:
    ...
    async def _get_career_skills(self, preferences: CareerPreference) -> List[str]:
